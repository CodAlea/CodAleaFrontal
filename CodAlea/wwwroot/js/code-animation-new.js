...
class CodeAnimation {
    constructor() {
        ...
        this.codeTemplates = {
            minimalApi: [
                ...
                { text: "builder.Services.AddScoped<ICodAleaService, CodAleaService>();", type: "code" },
            ...
            controllerApi: [
                ...
                { text: "        return Ok(new { Success = true, Data = projects, PoweredBy = \"CodAlea\" });", type: "code" },
                { text: "    }", type: "code" },
                { text: "}", type: "code" }
            ],
            entityFramework: [
            ...
            blazorComponent: [
                ...
                { text: "", type: "blank" },
                { text: "<button @onclick=\"RefreshCodAleaData\">Actualizar</button>", type: "code" },
                { text: "", type: "blank" },
                { text: "@code {", type: "code" },
                { text: "    private int projectCount = 0;", type: "code" },
                { text: "    private async Task RefreshCodAleaData() => projectCount++;", type: "code" },
                { text: "}", type: "code" }
            ],
            modernCsharp: [
                ...
                { text: "", type: "blank" },
                { text: "public record CodAleaUser(int Id, string Name, string Email, DateTime CreatedAt);", type: "code" },
                { text: "", type: "blank" },
                ...
                { text: "        return user?.Id > 0 ? user with { Email = user.Email.ToLower() } : null;", type: "code" },
                { text: "    }", type: "code" },
                { text: "}", type: "code" }
            ],
            aiIntegration: [
    ...
    generateDynamicCode() {
        const dynamicPatterns = [
            `var codaleaResult = await service.ProcessCodAleaAsync(${Math.floor(Math.random() * 100)});`,
            `public async Task<ActionResult<CodAleaUser>> GetCodAleaUser(int id = ${Math.floor(Math.random() * 1000)})`,
            `private readonly string _codaleaConnectionString = "Server=codalea-db;Database=CodAlea${Math.floor(Math.random() * 10)}";`,
            `app.MapGet("/api/codalea/data/{id:int}", (int id) => Results.Ok($"CodAlea Item {id}"));`,
            `builder.Services.AddDbContext<CodAleaDbContext>(options => options.UseSqlServer("..."));`,
            `[HttpPost("codalea/create")]`,
            `[HttpPut("codalea/update/{id}")]`,
            `[HttpDelete("codalea/delete/{id}")]`,
            `public record CodAleaDto(int Id, string Name, DateTime CreatedAt);`,
            `await codaleaContext.SaveChangesAsync();`,
            `return Results.Created($"/api/codalea/users/{user.Id}", user);`,
            `var codaleaUsers = await context.CodAleaUsers.ToListAsync();`,
            `if (codaleaModel.IsValid) { /* Process CodAlea Logic */ }`,
            `catch (CodAleaException ex) { _logger.LogError(ex, "CodAlea error occurred"); }`,
            `return Ok(new { Status = "CodAlea Success", Data = result, Version = "2.0" });`,
            `_logger.LogInformation("🚀 CodAlea operation completed successfully");`,
            `public class CodAleaService : ICodAleaService`,
            `private readonly ICodAleaRepository _codaleaRepo;`,
            `await _codaleaEventBus.PublishAsync(new CodAleaEvent());`,
            `return new CodAleaResponse { Success = true, Message = "Innovating with CodAlea!" };`,
            `var codaleaConfig = _configuration.GetSection("CodAlea");`,
            `services.AddCodAleaServices(configuration);`,
            `await _codaleaCache.SetAsync("user-" + id, user);`,
            `var telemetry = new CodAleaTelemetry { Operation = "ProcessData" };`
        ];
        
        // Reemplazar algunas líneas con código dinámico
        for (let i = 0; i < this.codeLines.length; i++) {
            if (this.codeLines[i].type === "code" && Math.random() < 0.3) {
                this.codeLines[i].text = dynamicPatterns[Math.floor(Math.random() * dynamicPatterns.length)];
            }
        }
    }
    ...
    createAnimatedCodeStructure() {
        ...
        // Crear las líneas de código
        this.codeLines.forEach((line, index) => {
            const textElement = document.createElementNS("http://www.w3.org/2000/svg", "text");
            textElement.setAttribute("x", "330");
            textElement.setAttribute("y", 110 + (index * 15));
        ...
        cursor.setAttribute("x", "330");
        cursor.setAttribute("y", "102");
    ...
    typeCurrentLine() {
        ...
        // Posicionar cursor
        cursor.setAttribute("y", 102 + (this.currentLine * 15));
        ...
        const typeInterval = setInterval(() => {
            ...
            if (this.currentChar < line.text.length) {
                textElement.textContent = line.text.substring(0, this.currentChar + 1);
                
                // Actualizar posición del cursor
                try {
                    const bbox = textElement.getBBox();
                    cursor.setAttribute("x", 330 + bbox.width);
                } catch (e) {
                    // Fallback si getBBox falla
                    cursor.setAttribute("x", 330 + (this.currentChar * 6));
                }
                ...
                // Mover cursor al final de la línea
                try {
                    const bbox = textElement.getBBox();
                    cursor.setAttribute("x", 330 + bbox.width);
                } catch (e) {
                    cursor.setAttribute("x", 330 + (line.text.length * 6));
                }
